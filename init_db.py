import logging
import time
from sqlalchemy import Column, String, Boolean, Text, ForeignKey, Integer, DateTime, func
...
# --- 테이블 설계 ---
# (Channel, Video, SearchLog, PopularIngredient 클래스 정의)
...
if __name__ == "__main__":
    ...
    Cloud DB에 연결하여 모든 기존 테이블을 삭제(DROP)하고,
    새로운 스키마로 테이블을 다시 생성(CREATE)합니다.
    (★경고★: 기존 데이터가 모두 삭제됩니다!)
    """
    ...
    try:
        logging.warning(
            "!!! 경고: 3초 후 Cloud DB의 모든 기존 테이블을 삭제(DROP)합니다... (중지: Ctrl+C)")
        time.sleep(3)  # 실수로 실행하는 것을 방지하기 위한 3초 대기

        logging.info("기존 테이블 삭제 중 (drop_all)...")
        Base.metadata.drop_all(engine)  # 1. 기존 테이블 모두 삭제

        logging.info("새 테이블 생성 중 (create_all)...")
        Base.metadata.create_all(engine)  # 2. 테이블 새로 생성

        logging.info("Cloud DB 테이블 초기화 및 생성 완료.")

    except Exception as e:
        logging.error(f"DB 초기화 중 오류 발생: {e}")
        logging.error(
