...
def get_channels(db: Session = Depends(get_db)):
    ...
    try:
        ...
        channel_list = [{"id": ch_id, "name": name} for ch_id, name in active_channels]
...
# --- ▲▲▲ Depends(get_db) 추가 ▲▲▲ ---
    ...
    try:
     ...
     .filter( # filter는 join 다음에
        ...
        recipe_list = [
            {"video_id": r.video_id, "title": r.title, "ai_ingredients": r.ai_ingredients, "channel_name": r.channel_name}
...
async def search_recipes(
    # 기존 Query 파라미터 유지
    keyword: str | None = Query(None, min_length=1, description="검색할 키워드 (제목 또는 재료)"),
    channel_id: str | None = Query(None, description="특정 채널 ID (선택 사항)"),
    # DB 세션 의존성 추가 (필수)
    db: Session = Depends(get_db)
):
    """ai_title 또는 ai_ingredients에서 키워드로 레시피를 검색합니다. (채널 필터링 추가)"""
    logging.info(f"API: /api/search 호출됨 (keyword='{keyword}', channel_id='{channel_id}')")
    try:
        # --- ▼▼▼ 함수 본문 (try 블록 내부) ▼▼▼ ---
     ...
     .filter( # filter는 join 다음에
        ...
        if keyword:
            ...
            query = query.filter(
                or_(
                    Video.ai_title.contains(keyword),
...
def get_channels(db: Session = Depends(get_db)):
    ...
    try:
        ...
        channel_list = [{"id": ch_id, "name": name} for ch_id, name in active_channels]
        logging.info(f"API: /api/channels - {len(channel_list)}개 채널 반환") # 로그 추가
        return channel_list 
    except Exception as e:
    ...
    try:
        ...
        query = query.filter(
            (Video.ai_title.like(search_term)) | (Video.ai_ingredients.like(search_term))
        )
        ...
        for video in search_results:
            ...
            results.append({
                ...
                "title": video.ai_title or video.title,
                "main_ingredients": ingredients_data.get("main", []),
                "sauce_ingredients": ingredients_data.get("sauce", [])
            })
        return results
    except Exception as e:
